...
2. **bun Packages**: Include @modelcontextprotocol/sdk, zod@3, and either express (for HTTP) or stdio support
...
- Initialize with `bun init` and create package.json
...
- Explain how to run the server (`bun start` or `npx tsx server.ts`)
...
_DevMultiplier Academy - Generate a complete MCP server project in TypeScript with tools, resources, and proper
configuration_

...
name: 'TypeScript MCP Server Instructions'
...
- Use the **@modelcontextprotocol/sdk** bun package: `bun install @modelcontextprotocol/sdk`
...
- Use **zod** for input/output schema validation: `bun install zod@3`
...
- Dynamic updates: call `.enable()`, `.disable()`, `.update()`, or `.remove()` on registered items to emit `listChanged`
  notifications
...
_DevMultiplier Academy - Instructions for building Model Context Protocol (MCP) servers using the TypeScript SDK_

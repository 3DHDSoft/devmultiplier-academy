...
export const metadata: Metadata = {
  ...
  description: 'Explore our comprehensive courses on Domain-Driven Design, CQRS, database optimization, REST APIs, and AI-assisted development.',
};
    ...
    <div className="bg-white py-8 sm:py-12 dark:bg-[#0d1117]">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mx-auto max-w-2xl text-center">
          <h1 className="text-4xl font-bold tracking-tight text-[#1f2328] sm:text-5xl dark:text-[#e6edf3]">Our Courses</h1>
          <p className="mt-4 text-lg text-[#656d76] dark:text-[#848d97]">Comprehensive, expert-led courses designed to transform how you architect, build, and ship production software.</p>
        </div>

        {/* Course list */}
        <div className="mx-auto mt-16 max-w-4xl space-y-8">
          {courses.map((course) => (
            <Link key={course.id} href={`/courses/${course.id}`} className="group flex flex-col rounded-2xl border border-[#d1d9e0] bg-white p-6 transition-all hover:border-[#0969da] hover:shadow-lg sm:flex-row sm:items-start sm:gap-6 dark:border-[#30363d] dark:bg-[#161b22] dark:hover:border-[#4493f8]">
              {/* Icon */}
              ...
              <div className="flex-grow">
                <div className="flex items-start justify-between">
                  <h2 className="text-xl font-semibold text-[#1f2328] group-hover:text-[#0969da] dark:text-[#e6edf3] dark:group-hover:text-[#4493f8]">{course.title}</h2>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {course.topics.map((topic) => (
                    <span key={topic} className="inline-block rounded-full border border-[#d1d9e0] bg-[#f6f8fa] px-3 py-1 text-xs font-medium text-[#1f2328] dark:border-[#30363d] dark:bg-[#21262d] dark:text-[#e6edf3]">
                      {topic}
                    </span>
                  ))}
                </div>

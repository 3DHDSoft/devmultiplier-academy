...
export function FAQAccordion({ items, className }: FAQAccordionProps) {
  ...
  return (
    <div
      className={cn(
        'divide-y divide-[#d1d9e0] rounded-md border border-[#d1d9e0] dark:divide-[#30363d] dark:border-[#30363d]',
        className
      )}
    >
      {items.map((item, index) => (
        ...
        >
          <button
            ...
            className="flex w-full items-center justify-between px-6 py-4 text-left transition-colors hover:bg-[#f6f8fa] dark:hover:bg-[#21262d]"
            aria-expanded={openIndex === index}
          >
            <span className="pr-4 font-medium text-[#1f2328] dark:text-[#e6edf3]">{item.question}</span>
            <ChevronDown
              className={cn(
                'h-5 w-5 shrink-0 text-[#656d76] transition-transform duration-200 dark:text-[#848d97]',
                openIndex === index && 'rotate-180'
              )}
            />
          </button>
          <div
            className={cn('overflow-hidden transition-all duration-200', openIndex === index ? 'max-h-96' : 'max-h-0')}
          >

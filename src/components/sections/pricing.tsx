          ...
          {tiers.map((tier) => (
            <div
              ...
              className={`relative flex flex-col rounded-2xl p-8 ${
                tier.featured
                  ? "border-2 border-blue bg-navy text-white shadow-xl"
                  : "border border-light-gray bg-white"
              }`}
            >
              {tier.featured && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 rounded-full bg-blue px-4 py-1 text-xs font-semibold text-white">
                  Most Popular
                </div>
              )}
                ...
                {tier.period && (
                  <span
                    className={`ml-1 text-sm ${
                      tier.featured ? "text-blue-200" : "text-slate"
                    }`}
                  >
                    {tier.period}
                  </span>
              ...
              <p
                className={`mt-4 text-sm ${
                  tier.featured ? "text-blue-100" : "text-slate"
                }`}
              >
                {tier.description}
              </p>
                  ...
                  <li key={feature} className="flex items-start">
                    <Check
                      className={`mr-2 h-5 w-5 flex-shrink-0 ${
                        tier.featured ? "text-cyan" : "text-blue"
                      }`}
                    />
                    <span
                      className={`text-sm ${

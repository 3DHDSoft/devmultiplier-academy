      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-lg text-slate">
            Five comprehensive courses designed to transform how you architect,
        ...
        <div className="mx-auto mt-16 grid max-w-5xl gap-8 md:grid-cols-2 lg:grid-cols-3">
          {courses.map((course) => (
            <Link
              ...
              className="group relative flex flex-col rounded-2xl border border-light-gray bg-white p-6 transition-all hover:border-blue hover:shadow-lg"
            >
              {/* Icon */}
              <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-lg bg-navy text-white transition-colors group-hover:bg-blue">
                <course.icon className="h-6 w-6" />
              </div>

              {/* Title */}
              <h3 className="text-lg font-semibold text-navy group-hover:text-blue">
                {course.title}
              </h3>

              {/* Description */}
              <p className="mt-2 flex-grow text-sm text-slate">
                {course.description}
              </p>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {course.topics.map((topic) => (
                  <span
                    key={topic}
                    className="inline-block rounded-full bg-off-white px-2.5 py-0.5 text-xs font-medium text-slate"
                  >
                    {topic}
                  </span>
                ))}
              ...
              {/* Arrow */}
              <div className="mt-4 flex items-center text-sm font-medium text-blue opacity-0 transition-opacity group-hover:opacity-100">
